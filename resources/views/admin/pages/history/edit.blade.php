...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('admin.pages.history.update', $history->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('title_ru') is-invalid @enderror" 
                       id="title_ru" name="title_ru" value="{{ old('title_ru', $history->title_ru) }}">
                @error('title_ru')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control ckeditor @error('content_ru') is-invalid @enderror" 
                          id="content_ru" name="content_ru">{{ old('content_ru', $history->content_ru) }}</textarea>
                @error('content_ru')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('title_en') is-invalid @enderror" 
                       id="title_en" name="title_en" value="{{ old('title_en', $history->title_en) }}">
                @error('title_en')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control ckeditor @error('content_en') is-invalid @enderror" 
                          id="content_en" name="content_en">{{ old('content_en', $history->content_en) }}</textarea>
                @error('content_en')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
